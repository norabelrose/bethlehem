...
class HebrewCalendarResult:
    ...
    def __init__(self, calendar: list, ts,
                 loc_name: str, loc_lat: float, loc_lon: float,
...
""")
    ...
    def print_key_events(self):
        """
        Cross-reference key Star-of-Bethlehem candidate events against the
        computed Hebrew calendar.  Only printed when the range covers 3–2 BC.
        """
        if not (self.start_year <= -2 and self.end_year >= -1):
            return
        ...
        for label, t_event in events:
            hmo, am_yr, hday = self.hebrew_date_for_jd(t_event.tt)

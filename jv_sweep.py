...
parser.add_argument(
    "--lat", type=float, metavar="DEG", help="Custom latitude in degrees N"
)
parser.add_argument(
    "--lon", type=float, metavar="DEG", help="Custom longitude in degrees E"
)
parser.add_argument(
    "--name",
    type=str,
...
def bisect_sep(
    jd_lo: float, jd_hi: float, entering: bool, threshold: float = ARCMIN
) -> float:
...
windows = [
    (jd_all[max(0, s - 2)], jd_all[min(n - 1, e + 1)]) for s, e in zip(starts, ends)
]
print(
...
for jd_lo, jd_hi in windows:
    ...
    sky_cond = (
        "night" if alt_sun_c < NIGHT_ALT else f"daytime (sun alt {alt_sun_c:+.1f}°)"
    )
...
print(
    f"  Visibility : elongation > {AV_JUP}°,"

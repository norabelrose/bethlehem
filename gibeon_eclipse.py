...
def bisect_contact(
    ts, earth, moon_body, sun_body, t_lo, t_hi, threshold_fn, rising=True, tol_min=0.5
):
    """
...
def main():
    ...
    n_ecl = len(t_ecl)
    print(
        f"  Umbral eclipses: {n_ecl}  ({sum(y_ecl==2)} total, {sum(y_ecl==1)} partial)\n"
    )

    # -----------------------------------------------------------------------
    # Step 2: process each eclipse
    # -----------------------------------------------------------------------
    def umbra_f(ca, ur, mr):
        return ur + mr - ca  # > 0 when overlapping umbra

    def total_f(ca, ur, mr):
        return ur - mr - ca  # > 0 when fully inside umbra

    results = []
    print("Checking horizon visibility (this may take several minutes)…", flush=True)
    ...
    for idx in range(n_ecl):
        ...
        prev_i, prev_illum, prev_vmag = moon_phase_info(
            earth, moon_body, sun_body, t_prev
        )
        ...
        t_u1 = bisect_contact(
            ts, earth, moon_body, sun_body, t_lo, t_fm, umbra_f, rising=True
        )
        t_u4 = bisect_contact(
            ts, earth, moon_body, sun_body, t_fm, t_hi, umbra_f, rising=False
        )
        t_u2 = t_u3 = None
        if etype == "Total":
            t_u2 = bisect_contact(
                ts, earth, moon_body, sun_body, t_u1, t_fm, total_f, rising=True
            )
            t_u3 = bisect_contact(
                ts, earth, moon_body, sun_body, t_fm, t_u4, total_f, rising=False
            )
        ...
        results.append(
            dict(
                ...
                t_u4=t_u4,
                eclip_dur=eclip_dur,
                t_vs=ts.tt_jd(vs_tt_ecl),
                t_ve=ts.tt_jd(ve_tt_ecl),
                vis_dur=vis_dur,
                ms_alt=ms_alt,
                ms_az=ms_az,
                ss_alt=ss_alt,
    ...
    TIER_META = {
        ...
        "POSSIBLE": (
            f"POSSIBLE    (|δ| ≤ {SIGMA_1H:.1f} h,  ±{SIGMA_1H*15:.0f}° longitude)",
            possible,
        ),
        "SPECULATIVE": (
            f"SPECULATIVE (|δ| ≤ {SIGMA_2H:.1f} h,  ±{SIGMA_2H*15:.0f}° longitude)",
            speculative,
        ),
    }
    ...
    print(
        f"  Terrain: western horizon (Aijalon valley) depressed {AIJALON_DEP_DEG}°  "
        f"(az {AIJALON_AZ_LO:.0f}°–{AIJALON_AZ_HI:.0f}°)"
    )
    print(
        f"  ΔT uncertainty model: ±{SIGMA_1H:.0f} h (tier 1), ±{SIGMA_2H:.0f} h (tier 2)"
    )
    print("=" * 72)
    print()
    print(f"  {'NOMINAL':15s} {len(certain):4d} events  (nominal ΔT)")
    print(
        f"  {'POSSIBLE':15s} {len(possible):4d} additional events  (|δ| ≤ {SIGMA_1H:.0f} h, ≡ ≤{SIGMA_1H*15:.0f}° lon)"
    )
    print(
        f"  {'SPECULATIVE':15s} {len(speculative):4d} additional events  (|δ| ≤ {SIGMA_2H:.0f} h, ≡ ≤{SIGMA_2H*15:.0f}° lon)"
    )
    ...
    for tier_key in ("NOMINAL", "POSSIBLE", "SPECULATIVE"):
        ...
        for r in tier_list:
            ...
            print()
            print(
                f"  Event {event_n}: {r['etype'].upper()} LUNAR ECLIPSE  |  "
            ...
            if tier_key != "NOMINAL":
                ...
                print(
                    f"  (≡ observer {abs(d*15):.0f}° {'west' if d>0 else 'east'} of Gibeon "
                    f"→ effective longitude {eff_lon:.1f}°E)"
                )

            print()
            print("  Eclipse contacts (TT):")
            ...
            print("  Azimuths at window end:")
            print(f"    Moon: az {r['me_az']:6.1f}°  alt {r['me_alt']:+5.1f}°")
            print(f"    Sun:  az {r['se_az']:6.1f}°  alt {r['se_alt']:+5.1f}°")
            print()
            print("  Moon at eclipse maximum:")
            print(
                f"    Angular diameter: {r['moon_diam_fm']:.1f}′  "
                f"({'large' if r['moon_diam_fm'] >= 32 else 'small'} disc)"
            )
            print("  Moon on preceding evening (~24 h before max eclipse):")
            print(f"    Angular diameter: {r['prev_diam']:.1f}′")
            print(
                f"    Phase angle:      {r['prev_i']:.1f}°  "
                f"(illumination {r['prev_illum']*100:.1f}%)"
            )
            print(
                f"    Apparent magnitude ≈ {r['prev_vmag']:.1f}  "
